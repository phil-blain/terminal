    ...
    _renderer{ renderer }
{
    ...
    const auto stride = static_cast<size_t>(screenBufferSize.X) * sizeof(uint16_t);
    const auto rows = static_cast<size_t>(screenBufferSize.Y);
    const auto halfSize = stride * rows;
    auto buffer = static_cast<std::byte*>(VirtualAlloc(nullptr, 2 * halfSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE));
    ...
    {
        _storage.emplace_back(reinterpret_cast<wchar_t*>(buffer), reinterpret_cast<uint16_t*>(buffer + halfSize), screenBufferSize.X, _currentAttributes);
        buffer += stride;
    }
                                 ...
                                 const TextAttribute attr)
{
    ...
    if (fSuccess)
    {
        ...
        try
        {
            Row.Replace(iCol, dbcsAttribute.IsSingle() ? 1 : 2, chars);
        }
        catch (...)
        {
            LOG_HR(wil::ResultFromCaughtException());
...
[[nodiscard]] NTSTATUS TextBuffer::ResizeTraditional(const COORD newSize) noexcept
{
    ...
    const auto stride = static_cast<size_t>(newSize.X) * sizeof(uint16_t);
    const auto rows = static_cast<size_t>(newSize.Y);
    const auto halfSize = stride * rows;
    auto buffer = static_cast<std::byte*>(VirtualAlloc(nullptr, 2 * halfSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE));
    ...
    try
    {
        ...
        while (_storage.size() < static_cast<size_t>(newSize.Y))
        {
            _storage.emplace_back(nullptr, nullptr, newSize.X, attributes);
        }
        ...
        // and cleanup the UnicodeStorage characters that might fall outside the resized buffer.
        {
            ...
            {
                it.Resize(reinterpret_cast<wchar_t*>(data), reinterpret_cast<uint16_t*>(data + halfSize), newSize.X);
                data += stride;
            }
        }
                           ...
                           std::optional<std::reference_wrapper<PositionInformation>> positionInfo)
{
    ...
    for (; iOldRow < cOldRowsTotal; iOldRow++)
    {
        ...
        // character)
        short iOldCol = 0;
        auto chars{ row.CharsBegin() };
        auto attrs{ row.AttrBegin() };
...
uint16_t TextBuffer::GetHyperlinkId(std::wstring_view uri, std::wstring_view id)
{
    ...
    else
    {
        ...
        std::wstring newId;
        newId.reserve(id.size() + uri.size());
        newId.append(id);
        newId.append(uri);
